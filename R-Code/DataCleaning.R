...
### Some NAs deliberately introduced by coercion, warnings can be ignored
...
# Store information on Emergency Department Volume (categorical variable)
edv <- tec19[tec19$Measure.ID == "EDV", c(1,12)]

# Capitalize Emergency Department Volume level names 
...
# Now that non-numeric information has been extracted, set all non-numeric values to NA
tec18$Score <- ifelse(grepl('[0-9]', tec18$Score), as.numeric(tec18$Score), NA)

# Store Facility IDs of all hospitals for later merge
ids <- data.frame(Facility.ID = unique(tec18$Facility.ID[order(tec18$Facility.ID)]))

# Extract and store length of stay data
LOS <- reshape(tec18[tec18$Measure.ID %in% c("ED_1b", "ED_2b", "OP_18b", "OP_18c", "OP_22"),c(1,10,12)],
                 timevar = "Measure.ID", idvar = "Facility.ID", direction = "wide")

# Convert "Left before being seen" from percentage to proportion
LOS$Score.OP_22 <- LOS$Score.OP_22 / 100
...
# Convert Facility ID to numeric and rename column 
...
# Exclude Puerto Rico zip codes (which begin with "00")
serv <- subset(serv, ZIP_CODE > 1000)

# Format zip codes to include a single leading zero, when needed for certain east coast zip codes
...
# Create subset of relevant columns only and exclude Puerto Rico zip codes
...
# Convert demographic info to proportions of each race/ethnicity in each zip code 
race[,2:11] <- sapply(race[,2:11], as.numeric)
race[,3:9] <- sapply(race[,3:9], function(x) { x / race[,2]})
race <- race[,-2]

# Rename variables in preparation for merge
...
# Merge data files together one by one
...
hospital_data <- cbind(LOS, bigmerge, identifying_info)
...
# Output cleaned data
write.csv(hospital_data, "CleanedData.csv", row.names = FALSE)
