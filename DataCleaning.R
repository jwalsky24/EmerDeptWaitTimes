...
pairs
na.action(na.omit)
test1[test1 == "Not Available"] <- NA
sum(complete.cases(test2))

mean(na.omit(test2$Score.ED_2b[test2$edv == "low"]))
mean(na.omit(test2$Score.ED_2b[test2$edv == "medium"]))
mean(na.omit(test2$Score.ED_2b[test2$edv == "high"]))
mean(na.omit(test2$Score.ED_2b[test2$edv == "very high"]))

hist(test2$Score.OP_18c[test2$edv == "medium"])
hist(test2$Score.OP_18c[test2$edv == "high"])
hist(test2$Score.OP_18c[test2$edv == "very high"])

str(test2)

test3 <- test2
test3$NonWhite <- test3
str(test3)

gam1 <- gamlss(TimeInER ~ Black + Asian + Hispanic, family = GA, data = na.omit(hospital_data))
summary(gam1)



descdist(as.numeric(na.omit(test2$Score.ED_1b)))

tdata <- as.numeric(na.omit(test2$Score.OP_18b))

fit.weibull <- fitdist(tdata, "weibull")
fit.gamma <- fitdist(tdata, "gamma")
fit.normal <- fitdist(tdata, "norm")
fit.poi <- fitdist(tdata, "pois")
...
b1 <- na.omit(subset(test2, White > 0.8, select = "Score.OP_18c"))
b2 <- na.omit(subset(test2, White < 0.8, select = "Score.OP_18c"))

summary(lm(Score.OP_18b ~ White, data = test2))

mean(b1$Score.OP_18c)
mean(b2$Score.OP_18c)
...
sum(complete.cases(test2$Score.OP_18b))

as.numeric(na.omit(test2$Score.OP_18b))
